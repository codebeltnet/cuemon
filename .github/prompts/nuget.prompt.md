...
- For every file matching `.nuget/**/PackageReleaseNotes.txt`:
  ...
  3. If not found within the first 3 lines, do nothing for that file.
     a. Do not assume that each file are the same - process each file independently - THIS IS A REQUIREMENT - DO NOT GROUP AND DO NOT ASSUME CONTENT IS THE SAME ACROSS FILES.
  4. Apply the template exactly as shown, preserving all whitespace and blank lines - DO NOT ADD AN EXTRA NEWLINE!
