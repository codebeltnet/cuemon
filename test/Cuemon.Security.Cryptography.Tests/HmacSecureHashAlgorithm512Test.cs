...
{
    public class HmacSecureHashAlgorithm512Test : Test
    {
        public HmacSecureHashAlgorithm512Test(ITestOutputHelper output) : base(output)
        {
        }

        [Fact]
        public void ComputeHash_ShouldReturnExpectedHmacSha512_ForGivenInput()
        {
            var secret = "unittest-secret"u8.ToArray();
            var sut = new HmacSecureHashAlgorithm512(secret, null);
        ...
        [Fact]
        public void ComputeHash_ShouldReturnExpectedHmacSha512_ForEmptyArray()
        {
        ...
        {
            byte[] secret = null;
            var sut = Record.Exception(() => new HmacSecureHashAlgorithm512(secret, null));
            Assert.Null(sut);
        }
