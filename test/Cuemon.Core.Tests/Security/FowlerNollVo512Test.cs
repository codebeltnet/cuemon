...
{
    public class FowlerNollVo512Test : Test
    {
        public FowlerNollVo512Test(ITestOutputHelper output) : base(output) { }

        [Fact]
        public void Constructor_Default_SetsBits()
        {
            var sut = new FowlerNollVo512();
            Assert.Equal(512, sut.Bits);
        }

        [Fact]
        public void ComputeHash_Empty_ReturnsOffsetBasis_LengthIs64Bytes()
        {
            var sut = new FowlerNollVo512();
