...
{
    public class FowlerNollVo128Test : Test
    {
        public FowlerNollVo128Test(ITestOutputHelper output) : base(output) { }

        [Fact]
        public void Constructor_Default_SetsBits()
        {
            var sut = new FowlerNollVo128();
            Assert.Equal(128, sut.Bits);
        }

        [Fact]
        public void ComputeHash_Empty_ReturnsOffsetBasis_WithSelectedEndianness()
        {
            var sut = new FowlerNollVo128();
            var result = sut.ComputeHash(Array.Empty<byte>()).GetBytes();
            Assert.Equal(16, result.Length);

            var sutLe = new FowlerNollVo128(o => o.ByteOrder = Endianness.LittleEndian);
            var resultLe = sutLe.ComputeHash(Array.Empty<byte>()).GetBytes();
            Assert.Equal(16, resultLe.Length);
        }
